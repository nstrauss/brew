...
The macOS `.pkg` installer also installs Homebrew to its default prefix (`/opt/homebrew` for Apple Silicon and `/usr/local` for macOS Intel) for the same reasons as above. It's available on [Homebrew/brew's latest GitHub release](https://github.com/Homebrew/brew/releases/latest). Optionally define a user with the environment variable  `HOMEBREW_PKG_USER`. For example, `HOMEBREW_PKG_USER="brew_user" installer -pkg Homebrew.pkg -target /`
